...
import { feedbackTools, executeTool } from "../../../lib/agent-tools";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  while (true) {
    ...
    if (response.stop_reason === "tool_use") {
      ...
      for (const block of response.content) {
        ...
        if (block.name === "insert_feedback") {
          feedback_inserted++;
          const input = block.input as { source_type: string; score: number };
          send({ type: "feedback_inserted", source_type: input.source_type, score: input.score });
        }
  ...
  send({ type: "done", feedback_inserted });
  res.end();
}
